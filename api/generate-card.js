...
export default async function handler(req, res) {
    ...
    try {
        ...
        // ===== ВАШ УЛУЧШЕННЫЙ ПРОМПТ С ПРАВИЛАМИ WILDBERRIES =====
        const basePrompt = `Ты — ведущий дизайнер инфографики для Wildberries. Твоя задача создать фото-карточку товара, которая привлечет максимум внимания, увеличит продажи и строго соответствует всем правилам маркетплейса.
...
#### 1. ПОИСК РЕАЛЬНЫХ ХАРАКТЕРИСТИК
На основе названия "${productName}", найди в своей базе данных или через поиск реальные характеристики, технические детали и преимущества этого товара. Добавь их на карточку в виде иконок или коротких надписей. Используй ТОЛЬКО реальные данные.

#### 2. ЗАПРЕЩЁННЫЙ КОНТЕНТ (строгое соблюдение правил Wildberries)
- НЕ используй фразы: "хит", "лучший из всех", "лидер продаж", "топ", "номер 1", "бестселлер", "самый популярный"
- НЕ добавляй на карточку цены (кроме основной цены товара), QR-коды, скидки, промокоды, контакты, ссылки на сайты
- НЕ используй призывы к действию (позвони, напиши, сравни, купи прямо сейчас, закажи)
- НЕ указывай количество проданных товаров ("продано 1000 штук")
- НЕ добавляй вознаграждение за отзыв ("скидка за отзыв")
- НЕ используй оценочные суждения ("отличный", "превосходный", "идеальный")
...
Создай фото-карточку, от которой невозможно оторвать взгляд, полностью соответствующую правилам Wildberries.`;
        ...
        const variation = `\n\nЭто попытка №${attempt + 1} из 5. Сделай этот вариант отличным от предыдущих: используй другое расположение текста, другую цветовую гамму или композицию, но обязательно сохрани все ключевые элементы и реальные характеристики товара.`;
