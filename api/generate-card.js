...
/**
 * УНИВЕРСАЛЬНЫЙ/КЛАССИЧЕСКИЙ ПРОМПТ (с правилами Wildberries)
 */
function getBasePrompt(productName, brand, price, userFeatures, attempt) {
    ...
    // Визуальные требования (из правил WB)
...
#### 2. ВИЗУАЛЬНЫЕ ТРЕБОВАНИЯ (строго соблюдай):
${visualRequirements}
...
Это попытка №${attempt + 1} из 5. Вариант должен отличаться от предыдущих (другая композиция, цветовая гамма или расположение текста), но сохранять все ключевые элементы товара.`;
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // ✅ ИСПОЛЬЗУЕМ НОВЫЙ УНИВЕРСАЛЬНЫЙ ПРОМПТ
        const finalPrompt = getBasePrompt(productName, brand, price, userFeatures, attempt);
