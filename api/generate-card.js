    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                googleSearch: {
                    enable: true  // Search Grounding включён [citation:2]
                }
            }
        });
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Добавляем указание использовать поиск в интернете
...
#### 1. ПОИСК РЕАЛЬНЫХ ХАРАКТЕРИСТИК (ВАЖНО!)
На основе названия "${productName}", используй Search Grounding для поиска в интернете реальных характеристик, технических деталей и преимуществ этого товара. Найди актуальную информацию о модели, технических спецификациях, материалах, размерах и других важных параметрах. Используй только реальные данные, найденные в поиске.
...
- **По бокам:** Ключевые фишки в виде иконок с подписями (используй реальные характеристики, найденные в интернете).
...
Создай фото-карточку, от которой невозможно оторвать взгляд, полностью соответствующую правилам Wildberries. Используй реальные характеристики товара, найденные в интернете через поиск.`;
        ...
        console.log('✅ Успешно сгенерировано изображение 900x1200 с реальными характеристиками');
        res.status(200).json({ 
            images: [publicUrl], 
            descriptions,
