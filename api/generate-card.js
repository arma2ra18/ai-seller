...
// Отключаем встроенный парсер Vercel (нужен для formidable)
export const config = {
    api: {
        bodyParser: false,
    },
...
/**
 * Генерация описаний товара (можно тоже через GigaChat, но пока заглушка)
 */
async function generateDescriptions(productName, brand, features, platform) {
    ...
    return [
        `Превосходный ${productName} от бренда ${brand}. Особенности: ${features}. Идеально подходит для повседневного использования. Закажите сейчас!`,
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 3. Загруженные фото (пока не используем, но можно сохранить для истории)
        let photoArray = [];
        if (files.photos) {
        ...
        // Учитываем категорию и особенности
